...
| 角色 | 说明 |
|------|------|
| `user` | 普通用户，后续用于会议创建、查看、生成和发�?|
| `minutes_admin` | 纪要管理员，可管理提示词模板�?ASR 热词 |
| `system_admin` | 系统管理员，可管理系统配置、用户角色、审计日志和连接测试 |
...
| `/api/admin/prompt-templates`、`/api/admin/hotwords` | `minutes_admin` �?`system_admin` |
